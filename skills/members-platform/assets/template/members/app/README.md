...
Plano completo em `docs/plano-desenvolvimento.md`.
...
| `src/styles/kingdom-ui.css` | Estilos do design system. Vem do design system incluído na skill (`assets/design-system/assets/kingdom-ui.css`) |
...
Preencha com os serviços do projeto: projeto Vercel (pasta `members/app`), projeto Supabase (região), domínio Resend e remetente. Variáveis no Vercel: `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `RESEND_API_KEY`, `EMAIL_FROM`, `EMAIL_REPLY_TO`, `INVITE_TTL_DAYS`, `SUPPORT_EMAIL`, `SUPPORT_WHATSAPP`, `CRON_SECRET`, `GATEWAY_WEBHOOK_SECRET`, `GATEWAY_ACCEPT_TEST_EVENTS` e `DISPUTE_SUSPENDS_ACCESS`. Depois de mudar uma variável, é preciso publicar de novo.
...
- **Runbook de suporte:** [`docs/runbook-suporte.md`](../../docs/runbook-suporte.md).
...
- **Dados de exemplo:** `members/supabase/seed.sql` tem produtos de demonstração, usados pelos testes. Substitua-os pelos produtos reais (ou carregue-os no admin) antes do lançamento.
