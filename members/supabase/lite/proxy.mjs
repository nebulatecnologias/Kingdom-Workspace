...
http.createServer((req, res) => {
  ...
  if (!route) { res.writeHead(404, { "content-type": "application/json" }); res.end('{"message":"not available in lite stack"}'); return; }
  const path = req.url.slice(route[0].length) || "/";
  const up = http.request({ host: "127.0.0.1", port: route[1], path, method: req.method, headers: req.headers }, (r) => { res.writeHead(r.statusCode, r.headers); r.pipe(res); });
